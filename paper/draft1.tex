...
Even though such a ``black-box'' approach does not explicitly try to build an
interpretable model, per se, it could nonetheless help to greatly accelerate
our understanding of the complex interactions between the set of optimizations,
their parameters and the hardware architectural features.
% (XXX: that's what we are trying to do w/ ML-based, decision trees, etc.).
...
modeling techniques, rather than constructing a model manually. By learning the model, one can
