...
% HPC Landscape: large diversity of architectures, no consensus => push to
% flexibility => push to sw

Even though the future lies in low-power multi-core hardware designs, the field
...
programmed. As a consequence, these systems have been increasingly relying on
flexible but low-level software-controlled cache management and parallelism
\cite{BerkeleyViewOfHPC} in order to better control and understand the various
trade-offs among (e.g.) performance, reliability, energy efficiency, production
costs, etc. This evolution has profoundly altered the landscape of software
development: application programmers are now facing a large diversity of
low-level architectural features while being left with rapidly evolving tools
and techniques to write high-performance \emph{and} portable code.

% Better tools: motivation
