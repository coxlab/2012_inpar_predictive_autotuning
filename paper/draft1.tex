...
% HPC Landscape: large diversity of architectures, no consensus => push to flexibility => push to sw
...
% 1.1 Motivation
% Better tools: motivation (compilers/tools/libs can't keep up, it's too difficult)
...
from optimal. One of the reason is that kernels can produce staggeringly large
optimization spaces \cite{K. Datta et al. Stencil computation optimization and
auto-tuning on state-of-the-art multicore architectures}. The problem is
further compounded by the fact that these spaces can be highly discontinuous
\cite{RyooHwuCGO2008}, difficult to explore and holding quasi-optimal solutions
at the edge of ''performance cliffs'' (in part due to hard resource
restrictions, like the register file size).
...
% XXX: * empirical optimization: intro / advantages
% XXX: * empirical optimization: disadvantages
% XXX: * hybrid optimization (to counter disadvantages) ?
% XXX: * empirical optimization: w/ ML @ compile time (another way to “fix” empirical)
% XXX: * empirical optimization: ML @ runtime, especially for libs
% XXX: * empirical optimization: ML @ runtime for GPU libs why ? (w/ rtcg jit, rel. to pycuda, etc)
...
perform.
Our experiments consider a somewhat simplified setting in which the arguments are always stored with depth channels being contiguous in memory, followed by columns, then rows, and then filters having the largest stride.
...
With the gigaflops restriction, the argument space
included 602 configurations with between 1 and 50 gigaflops.
...
Our implementation space includes 16000 (XXX) points, too many to search exhaustively.
...
current high-end cards such as the GTX 580 and C2070.
...
the most promising part of the configuration space. This grid iterates over all
