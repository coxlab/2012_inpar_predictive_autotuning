...
\begin{figure}[!ht]
\centerline{
\begin{algorithm}{$\Algo{\color{darkred}{gpu\_fbcorr\_empirical}}\big(shapes, strides \big)$}
...
\begin{algorithm}{$\Algo{\color{darkred}{gpu\_fbcorr\_predictive}}\big(shapes, strides \big)$}
\Aitem $a \setto TaskFeatures(shapes, strides)$
\Aitem $c \setto PlatformFeatures()$
...
\Aitem \Return $b^*$
\end{algorithm}
\caption{Pseudo-code template for an auto-tuning implementation of
$\texttt{gpu\_fbcorr}$. Empirical auto-tuning (above) is inevitably slow
...
We show that the kernel from the GCG chapter can be modeled on a variety of hardware and get good performance compared to empirical auto-tuning. XXX numbers:
how good, how fast can prediction be done how much training data is necessary?
